...
let main argv = 
    ...
    let d2DRenderTarget = new RenderTarget(d2DFactory, surface, RenderTargetProperties(PixelFormat(Format.Unknown, SharpDX.Direct2D1.AlphaMode.Premultiplied)))
    ...
    RenderLoop.Run(form, fun _ ->
            ...
            let center = new SharpDX.Vector2(float32 500.0,float32 300.0)
            let scale = new SharpDX.Vector2(float32 (-rot+1.0),float32 (-rot+1.0))
...
//                    d2DRenderTarget.DrawLine(new Vector2(x1,y1),new Vector2(x2,y2), pinkBrush,0.3f) 
                ...
                for line,_ in lines do
                    match line with 
                        | None -> ()
                        | Some(l) -> printLine l 
//            d2DRenderTarget.Clear(new Nullable<Interop.RawColor4>(SharpDX.RawColor4(0.0f, 0.0f, 0.0f, 100.0f)))
